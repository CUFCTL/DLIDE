...
import argparse
import sys
caffe_root = '~/jtetrea/MM-ENet/caffe-mm-enet/'  # Change this to the absolute directory to SegNet Caffe
...
def drop_absorber_weights(model, net):
    ...
    for i, layer in enumerate(msg.layer):
        ...
        if layer.type == 'SpatialDropout':
            ...
            # get p from dropout layer
            p = msg.layer[i].spatial_dropout_param.dropout_ratio
            scale = 1/(1-p)
...
def bn_absorber_weights(model, weights):
    ...
    for i, layer in enumerate(msg.layer):
        ...
        if msg.layer[i].name == 'bn0_1' or msg.layer[i].name == 'bn0_1_D' :
            continue
...
def bn_absorber_prototxt(model):
    ...
    for i, l in enumerate(msg1.layer):
        if l.type == "BN":
            if msg1.layer[i].name == 'bn0_1' or msg1.layer[i].name == 'bn0_1_D':
                continue
...
def drop_absorber_prototxt(msg1):
    ...
    # search for bn layer and remove them
    for i, l in enumerate(msg1.layer):
        if l.type == "SpatialDropout" or l.type == "Python":
            msg1.layer.remove(l)
